...
(see [CancelJobError](src/bin/axum_error_example/api/handlers/cancel_job.rs#L16) for example).  
This allows a very precise description of the possible failure cases of each handler.  
Note that all of those handler error types have an
[InternalError](src/bin/axum_error_example/api/handlers/cancel_job.rs#L18)
variant which is here to handle server side errors (mapped to http 500 error code)
such as the database being unreachable. This variant is just a wrapper around
[anyhow::Error](https://docs.rs/anyhow/latest/anyhow/struct.Error.html) for
maximum flexibility.  
The other variants are user errors (mapped to http 4xx error code).

Because some handlers have some errors in common (for example
[JobNotFound](src/bin/axum_error_example/api/handlers/cancel_job.rs#L20)),
a global [ApiError](src/bin/axum_error_example/api.rs#L29) is used which contains
the union of all the variants of all the handler errors.  
This allows writing the [error message](src/bin/axum_error_example/api.rs#L36)
and the mapping to http error codes only once for each variant.  
Note that in this example the error is returned as a
[simple string](src/bin/axum_error_example/api.rs#L54), but one can easily use a more
advanced format such as [RFC7807](https://datatracker.ietf.org/doc/html/rfc7807).
...
[EnumInto](src/bin/axum_error_example/api/handlers/reset_job.rs#L15) derive macro,
avoiding the repetitive `From` manual implementation.
